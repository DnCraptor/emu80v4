...
// __scratch_x-функции возвращал исполнение во Flash.
...
static void __scratch_x("tv_main_loop") main_video_loopTV() {
    ...
    while (dma_inx_out != dma_inx) {
        ...
        /*
         * Прогрессивный 312-строчный RGB-растр без полустрок, второго поля
         * и NTSC-ветки. Строки 0..3 заняты кадровым синхроимпульсом,
         * framebuffer выводится 1:1 и центрируется внутри остального растра.
         */
        if (line_active < 4) {
            tv_fill8(output_buffer, v_mode.SYNC_TMPL, v_mode.H_len);
        } else {
            tv_fill8(output_buffer, v_mode.SYNC_TMPL, v_mode.sync_size);
            tv_fill8(output_buffer + v_mode.sync_size,
                     v_mode.NO_SYNC_TMPL,
                     v_mode.begin_img_shx - v_mode.sync_size);
            ...
            uint8_t* source =
                (source_y >= 0 && source_y < source_height)
                    ? getLineBuffer(source_y)
                    : NULL;

            if (!source) {
                tv_fill8(output_buffer,
                         v_mode.NO_SYNC_TMPL,
                         v_mode.img_size_x);
            } else {
                ...
                const int source_left =
                    (v_mode.img_size_x - source_width) / 2;
                const int shift_x = graphics_buffer.shift_x;

                for (int x = 0; x < v_mode.img_size_x; ++x) {
                    const int source_x =
                        x - source_left - shift_x;
                    ...
                    const uint8_t c = source[source_x];
                    *output_buffer++ =
                        map64colors[c & 0x3fu];
                }
            }
...
void tv_init(const output_format_e output_format) {
    ...
    /*
     * При clk_sys=448 МГц:
     *   CLK_SPD = 448 / 36 = 12.444444 МГц;
     *   делитель PIO = 18.0;
     *   H_len после выравнивания = 792.
     */
...
{
    (void)mode;
    menu_video_mode = GRAPHICS_VIDEO_KORVET;
}
