...
string DiskImage::chooseFileName(const string& title, bool* readOnly)
{
    return palOpenFileDialog(title, m_filter, false, readOnly);
}


void DiskImage::chooseFile(const string& title)
{
    string fileName = chooseFileName(title);
    if (fileName != "")
